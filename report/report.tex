...
We then propose \textbf{MahaKNN} --- a calibrated ensemble that combines
Mahalanobis Distance and $k$-NN scores via a convex combination with mixing
weight $\alpha$ selected on the validation set.
MahaKNN achieves an AUROC of \textbf{97.87\%} and FPR@95TPR of \textbf{8.30\%},
outperforming all individual detectors and surpassing the previous
state of the art.
...
\textbf{MahaKNN}, a simple yet effective ensemble that merges two
complementary detectors --- parametric (Mahalanobis Distance) and
non-parametric ($k$-NN) --- with a validation-set-calibrated mixing weight.
...
\subsection{Our Method: MahaKNN}
\label{sec:mahaknn}

Mahalanobis Distance and $k$-NN are complementary detectors: the former is
parametric and relies on a Gaussian assumption over the class-conditional
feature distribution, while the latter is non-parametric and makes no
distributional assumption. Their error patterns differ, and combining them
can improve over either individually.

We propose \textbf{MahaKNN}, a convex combination of the two scores:
\[
  s_\text{MahaKNN}(x) = \alpha \cdot \tilde{s}_\text{Maha}(x)
  + (1 - \alpha) \cdot \tilde{s}_{k\text{NN}}(x),
\]
where $\tilde{s}$ denotes a score standardised to zero mean and unit
standard deviation using validation-set statistics, and $\alpha \in [0,1]$
is selected by grid search on the validation set to minimise FPR@95TPR.
Standardisation ensures both scores are on the same scale before combination
without any leakage from the test set.
...
\begin{enumerate}
  ...
  \item Score the validation set with both detectors; compute normalisation
        statistics $(\mu, \sigma)$ for each score on the validation set.
  \item Grid-search $\alpha \in \{0.0, 0.05, \ldots, 1.0\}$; pick $\alpha^*$
        minimising FPR@95TPR on the validation set.
  \item At test time, standardise each score using the stored validation
...
For $k$-NN and MahaKNN we use $k=1$ (cosine similarity).
For MC Dropout we run $T=20$ passes with $p=0.1$ dropout.
The MahaKNN mixing weight $\alpha$ is grid-searched over 21 values in $[0,1]$
...
MahaKNN ensemble. Published results from \citet{podolskiy2021revisiting}
...
\midrule
\textbf{MahaKNN (ours)} & \textbf{97.87} & \textbf{8.30} & \textbf{91.45} \\
\bottomrule
\end{tabular}
\end{table}

MahaKNN consistently outperforms every individual detector on all three
metrics. The optimal $\alpha^*$ selected on the validation set was
$0.55$, indicating a slight preference for the Mahalanobis component.
The gain over Mahalanobis alone (AUROC $+$0.28\%, FPR $-$0.97\,pp)
confirms that the $k$-NN score provides complementary information even
after Mahalanobis has already captured the class-conditional geometry.
...
\textbf{MahaKNN} --- a calibrated ensemble of Mahalanobis Distance and
$k$-NN scores.
...
MahaKNN further improves upon the best individual detector, achieving
AUROC 97.87\% and FPR@95TPR 8.30\%, reducing the false-positive rate
nearly by half compared to the prior best published method.
...
Future work includes exploring contrastive pre-training objectives, learned
(rather than grid-searched) ensemble weights, and few-shot OOD exposure.
