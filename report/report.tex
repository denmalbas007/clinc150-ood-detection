...
Dropout. Mahalanobis Distance achieves the best AUROC of \textbf{97.59\%}
and FPR@95TPR of \textbf{9.27\%}, surpassing the previous state of the art.
...
structure by sweeping all 13 hidden states of BERT-base
(embedding layer + 12 Transformer blocks) and fitting a separate
class-conditional Gaussian at each layer.

Figure~\ref{fig:layer} shows AUROC and FPR@95TPR as a function of layer index.
Performance is low at early layers, rises steadily through the middle layers,
and peaks at layer \textbf{YY} (AUROC \textbf{YY.YY\%}, FPR@95TPR \textbf{YY.YY\%}),
which is not necessarily the last layer.
This suggests that the optimal feature layer for OOD detection in
fine-tuned BERT is task-dependent and should be treated as a hyperparameter.
...
Mahalanobis (ours) & \textbf{97.59} & \textbf{9.27} & \textbf{90.98} \\
...
All five of our methods surpass the previous state of the art of
\citet{podolskiy2021revisiting} (AUROC 96.76\%, FPR@95TPR 18.32\%).
Mahalanobis Distance achieves the best AUROC of 97.59\% and the lowest
FPR@95TPR of 9.27\%, nearly halving the false-positive rate of the prior
best method. The $k$-NN detector ($k$=1) is a close second (AUROC 97.58\%),
confirming that cosine-similarity-based retrieval in the BERT embedding space
is highly effective for OOD detection. Even the simplest baseline, MSP,
exceeds the published SotA with AUROC 96.50\%, which we attribute to the
stronger fine-tuning setup (larger batch, warmup scheduler, gradient clipping).

Our layer-wise analysis reveals that the optimal Mahalanobis feature layer
is not always the last one, suggesting that layer selection should be
treated as a hyperparameter in future work.

Future work includes exploring contrastive pre-training objectives,
score ensembling, and few-shot OOD exposure.
